...
public class dinningPhilo {
    ...
    static class Chopstick {
        ...
        void release() {
            // releases an acquire a permit and increases the number of available permits by
            // one
            mutex.release();
        }
    ...
    static class Philosopher extends Thread {
        ...
        void eat() {
            try {
                // determines the pseudorandom number between 0 to 1000 that represents the
                // sleep time in milli seconds
                int sleepTime = ThreadLocalRandom.current().nextInt(0, 1000);
                System.out.println("Philosopher " + (number + 1) + " eats for " + sleepTime + "ms"); // sleeps the
                                                                                                     // thread for a
                                                                                                     // specified time
